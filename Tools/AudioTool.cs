...
namespace TcgEngine
{
    ...
    public class AudioTool : MonoBehaviour
    {
        ...
        public double PlayMusic(string channel, AudioClip music, float vol = 0.3f, bool loop = true, double playDSPTime=0)
        {
            ...
            if (source)
            {
                if (!source.isPlaying || source.clip != music || playDSPTime!=0)
                {
                    source.clip = music;
                    ...
                    {
                        source.PlayScheduled(playDSPTime);
                        return playDSPTime;
                    }
                }
            }
