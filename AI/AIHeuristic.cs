...
namespace TcgEngine.AI
{
    ...
    public class AIHeuristic
    {
        ...
        public int available_mana_value = 10;   //Score of having mana available
        public int available_mana_usable_for_handcard_bonus = 5;   //Score of having mana available
        public int hand_card_value = 5;         //Score of having cards in hand
        public int kill_value = 35;              //Score of killing a card

        public int king_hp_value = 0;         //Score per player hp (pu necessaire, va etre changer pour le king)
        public int card_attack_value = 3;       //Score per board card attack
        public int card_hp_value = 2;           //Score per board card hp
        public int square_control_value = 3;    //Score per square controled
        public int attackable_piece_value = 6;        //Score per piece we can attack
        ...
        public int CalculateHeuristic(Game data, NodeState node, Player aiplayer, Player oplayer, int winscore)
        {
            ...
            score += aiplayer.king.hpOngoing * king_hp_value;
            score += aiplayer.GetTotalSquaresControlled(data) * square_control_value;
            score += aiplayer.GetTotalSquaresAttackable(data) * attackable_piece_value;
            ...
            score -= oplayer.king.hpOngoing * king_hp_value;
            score -= oplayer.GetTotalSquaresControlled(data) * square_control_value;
            score += oplayer.GetTotalSquaresAttackable(data) * attackable_piece_value;


            foreach (Card card in aiplayer.cards_board)
            {
                score += card.attack * card_attack_value;
            ...
            foreach (var mana in EnumExtensions.GetIndividualFlags(aiplayer.playerMana.GetAvailableMana()))
            {
                score += available_mana_value;
                foreach (var card in aiplayer.cards_hand)
                {
                    if (card.CardData.manaCost.HasFlag(mana))
                    {
                        score += available_mana_usable_for_handcard_bonus;
                    }
                }
            }
        ...
        public int CalculateActionScore(Game data, AIAction order)
        {
            ...
            if (order.type == GameAction.Attack)
            {
                ...
                int ascore = card.attack >= target.GetHP() ? 300 : 100; //Are you killing the card?
                //int oscore = target.GetAverageAttack() >= card.GetHP() ? -200 : 0; //Are you getting killed?
                return ascore + target.attack * 5;            //Always better to get rid of high-attack cards
            }
            ...
            if (order.type == GameAction.CreateMana)
            {
                return 300;
            }
        ...
        {
            if (order.type == GameAction.EndTurn)
                return 0; //End round can always be performed, 0 means any order
            ...
            if (order.type == GameAction.Attack)
                type_sort = 4; //Attacks
            if (order.type == GameAction.PlayCard && !is_spell)
                type_sort = 5; //Characters

            int card_sort = card==null ? 0 : card.Hash % 100;
