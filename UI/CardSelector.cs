        ...
        public void OnClickOK()
        {
            ...
            if (iability != null && data.selector == SelectorType.SelectorCard)
            {
                ...
                if (current_index >= 0 && current_index < card_img_list.Count)
                    selectorCard = card_img_list[current_index];
                if (selectorCard != null && selectorCard.card_index < card_list.Count)
                    selectedCard = card_list[selectorCard.card_index];

                Card caster = data.GetCard(data.selectorCasterUID);
                AbilityArgs args = new AbilityArgs() {ability = iability, caster = caster, target = selectedCard};
                if (selectedCard != null && iability.AreTargetConditionsMet(data, args))
                {
